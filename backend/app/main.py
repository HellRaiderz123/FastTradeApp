...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        from app.db.models_stock_holding import StockHolding  # noqa: F401 — stock holdings table
...
app.include_router(journal.router, dependencies=[Depends(require_authenticated_user)])
...
app.include_router(backtest.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(greeks.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(market.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(screener.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(options.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(options_real.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(suggestions.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(stock_suggestions.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(ws_positions.router)
app.include_router(alerts.router, dependencies=[Depends(require_authenticated_user)])
...
#  Phase 5 Features
app.include_router(notifications.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(websocket_routes.router)
app.include_router(health.router)
app.include_router(finance_router, dependencies=[Depends(require_authenticated_user)])
app.include_router(market_dashboard.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(swing_scanner.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(sentiment.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(news.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(twitter.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(economic_calendar.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(market_depth.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(config_routes.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(stock_news.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(timeframe_suggestions.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(peer_comparison.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(safety.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(candles.router, dependencies=[Depends(require_authenticated_user)])
...
app.include_router(position_suggestions.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(auto_trader_routes.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(trade_costs.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(watchlists.router, dependencies=[Depends(require_authenticated_user)])
app.include_router(condition_scanner.router, dependencies=[Depends(require_authenticated_user)])
...
app.include_router(ai_chat.router, dependencies=[Depends(require_authenticated_user)])

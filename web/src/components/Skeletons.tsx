...
export const SkeletonList: React.FC<{ items?: number }> = ({ items = 5 }) => (
  <div className="space-y-3">
    {Array.from({ length: items }).map((_, idx) => (
      <div key={idx} className="bg-gray-900 rounded-lg p-4 border border-gray-800 animate-pulse">
...
export const SkeletonGrid: React.FC<{ cols?: number; rows?: number }> = ({ 
  ...
  <div className={`grid grid-cols-1 md:grid-cols-${cols} gap-4`}>
    {Array.from({ length: cols * rows }).map((_, idx) => (
      <div key={idx} className="bg-gray-900 rounded-lg p-6 border border-gray-800 animate-pulse">
        <div className="space-y-3">
